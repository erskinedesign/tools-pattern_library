...
exports.metadata = function (file, page) {
    ...
    file.site     = {};
    file.modifiers = path.dirname(page._path).split(path.sep)[2];
...
exports.tree = function(filepath, config) {
    ...
    if (match.isDirectory()) {
        ...
        tree.children = fs.readdirSync(filepath).map(function(child){
            ...
            return exports.tree(filepath + '/' + child, config);
        });
    }

    // If the filepath isn’t a directory, try and grab file data from the config.files object and associate it.
    if(match.isFile()) {
        // Go through all of the config.files
